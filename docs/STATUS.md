...
### YouTube配信へのチャット転送（2026-07-30・実装完了／設定待ち）
- ワールド内の発言を、配信者アカウント名義でYouTubeライブのコメント欄へ流す
- **loyさんの作業が必要**: `GOOGLE_CLIENT_SECRET` と `PUBLIC_URL` をRenderに追加 →
  Google Cloudでスコープ追加とリダイレクトURI登録 → 画面から接続（手順は **SETUP_YOUTUBE.md**）
- 制約: **1日200回まで**（数秒ぶんをまとめて送る仕様で緩和済み）。増枠はGoogleへ申請
- 逆方向（YouTube→ワールド）は未実装。Unity側のWorker提供待ちが有力
...
- 自動テスト6本89項目がPASS（うちYouTube分はサーバー起動不要）
...
3. ~~ブロック／キック／BAN~~ **完了（2026-07-30）**。残るは
   **UnityのワールドデータをglTFで持ってくる**（仮ワールドの置き換え）。
   Unity側からデータを受け取るところから
...
6. ~~ポータル合流時にクロスポスト実装~~ **前倒しで実装済み（2026-07-30）**。
   制作順序が逆転したため、ポータルを待たずこちら側のGoogle認証で実装した。
   残るはVERSE COIN資金決済法の整理
