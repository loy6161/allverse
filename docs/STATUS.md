...
2026-07-29に「実戦で使うための3点（Googleログイン・管理者権限・イベント/ルーム）」を実装した。
残る大物は「Unity側presence連携の結合テスト」と、**loyさんによるGoogle/Tursoの設定**（SETUP_AUTH.md）。
...
6. ポイント制（設計のみ確定・実装保留）: 無料ポイントの獲得上限設計、有料ポイントは資金決済法の調査後
