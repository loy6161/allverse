...
## C-4. 入口の鍵（2026-07-31追加・**開発中に閉じておくためのもの**）

開発を進めている間、直リンク（`https://allverse.onrender.com/`）で来られたくないときに使います。

| Key | Value |
|---|---|
| `ENTRY_KEY` | 長めのランダムな文字列（例: `openssl rand -hex 12` の出力） |

設定すると、**`?k=<この値>` が付いていないアクセスは真っ白な404**になります（サービス名も案内文も出ません）。
案内するURLはこうなります。
...
短縮URL（short.gy）の転送先をこのURLにしておけば、配るのは短いURLのままで済みます。

### 閉じ方・開き方

| やりたいこと | 操作 |
|---|---|
| **閉じる** | Renderの `ENTRY_KEY` を別の値に変える → **配ったURLもブックマークも一斉に無効** |
| **開き直す** | 新しい鍵にして、short.gy の転送先を `?k=新しい鍵` に書き換える |
| **鍵をやめる（誰でも入れる）** | `ENTRY_KEY` を削除する |

### 知っておくこと

- ⚠ **これは「うっかり来た人を止める」ためのもので、秘密を守る仕組みではありません。**
  鍵つきURLをそのまま転送されれば入れます。守っているのは公開の可否であって中身ではありません
- **「short.gy 経由でしか入れない」は原理的に不可能です。** 302転送は「どこから来たか」をこちらに伝えず、
  転送後は本物のURLがアドレスバーに出ます（2026-07-31 実測で確認）。だから鍵方式にしました
- **VRChat連携（`/api/presence.json`）は鍵をかけていません。** ここを閉じるとWorkerが取れなくなり、
  VRCの客席からブラウザ勢が消えるためです
- 鍵はアドレスバーに出たままになります。気になる場合は別途相談してください
- WebSocketには鍵をかけていません。仕組みを知っている人は直接つなげます（うっかり防止が目的のため）
...
- `"entryGate": true` … 入口に鍵がかかっている（C-4を設定したときだけ）。
  「鍵を変えたのに反映されない」ときはここを見る（`/api/status` は鍵なしでも開きます）
