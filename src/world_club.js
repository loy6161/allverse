...
/**
 * 外壁の帯の厚み（m）。
 * ⚠ 1.4m では足りない（2026-08-08 レビュー指摘）。歩き4.2m/s × 走り6倍 × 車3.4倍 = 85m/s で、
 *   フレームが落ちて dt が上限(0.1秒)に張り付くと**1フレームで8.5m**動く。
 *   実測では抜けなかったが、loyさんの環境はGPUを使わない設定でフレームが落ちやすいので余裕を取る。
 *   あわせて「車に乗っている間は走れない」ようにもしてある（main.js）
 */
const WALL_T = 3.2;
...
export function createClubWorld(scene, { renderer } = {}) {
   ...
   *   床の粗さ0.45ではぼやけて像にならないため。
   *
   * ★ **さらに磨いた**（2026-08-04・loyさん「柱・壁が反射してる感じは全くないね」→
   *   3案のうち「床をもっと磨く」を選択）。**絵で見て決めた**。実測:
   *     粗さ0.18／強さ2.5／金属0.6 … 像1.00倍・明るさ1.00倍（光がのっぺり乗るだけ）
   *     **粗さ0.06／強さ3.0／金属0.8 … 像1.11倍・明るさ0.88倍**（採用。LEDの筋と柱の陰影が出る）
   *     粗さ0.03／強さ3.5／金属0.9 … 像1.17倍・明るさ0.82倍
   *     粗さ0.02／強さ4.0／金属1.0 … 像1.23倍・明るさ0.77倍（床が暗すぎる）
   *   ⚠ 磨くほど**床は暗くなる**（金属は拡散反射しないため）。明るさの落ちが1割強で
   *     像がはっきり出る 0.06 で止めた。
   *   ⚠ **数値のばらつきだけで判断しないこと。** 0.18 のときも「ばらつき2.1倍」だったが、
   *     絵で見ると**のっぺりした光が乗っていただけ**で柱は映っていなかった。
   */
  const FLOOR_MATERIAL = 'MI_Metal_TILE_Silver_2';
  /** 反射ONのときの床の質感。OFFに戻すときは matBase / applyMaterials 側の値へ戻す */
  const POLISHED = { roughness: 0.06, envMapIntensity: 3.0, metalness: 0.8 };
