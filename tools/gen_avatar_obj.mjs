...
function face() {
  // ⚠ 左右を別グループにする（2026-08-07）。VIPが左右で違う色を選べるようにするため。
  //   グループ名がそのままマテリアル名になる（obj_to_glb.py）ので、
  //   1グループにまとめると左右を別の色で塗れない。
  //   名前は**本人から見た左右**。画面では左右が入れ替わって見える（+x＝向かって右）
  for (const sx of [-1, 1]) {
    const side = sx > 0 ? '' : 'R'; // +x = 本人の左（従来の名前を残す）／-x = 本人の右
    faceQuad(`eye${side}`, sx * 0.105, -0.071, 0.041, 0.047);
    faceQuad(`eyec${side}`, sx * 0.105, -0.109, 0.041, 0.029, 0.0065);
    // ハイライトは白で固定（左右で分ける意味がないので1グループのまま）
...
const BANGS = {
  ...
  // ⚠ 深い側は -0.105 だと**目がほぼ隠れた**（2026-08-06 実測・loyさん「もうちょい浅くていい」）。
  //   目の上端は約 -0.048 なので、いちばん深いところを -0.06 にして眉のあたりで止める
  //   角度は**浅い側を上げて**付ける（深い側を下げると目が隠れるため。loyさん「も少し角度は付けていい」）
  partr: { bangs: 'blunt', bangYs: [0.035, 0.025, 0.0, -0.025, -0.045, -0.06, -0.045] },
  partl: { bangs: 'blunt', bangYs: [-0.045, -0.06, -0.045, -0.025, 0.0, 0.025, 0.035] },
};
...
const PARTS = {
  ...
  acc_tail() {
    ...
    // ⚠ 太さは 0.075/0.045 → 0.04/0.024（2026-08-07・loyさん「もっと細くして」）。
    //   だいたい半分。付け根と先の比は元のまま
    cone3('hair', [0, 0.34, -0.11], [0.02, 0.14, -0.3], 0.04);
    cone3('hair', [0.02, 0.15, -0.29], [0.06, 0.17, -0.44], 0.024);
  },
  acc_wing() {
